...
<form id="novo-exercicio" class="form-horizontal" role="form" style="margin-left: 20px">
	{{ csrf_field() }}
			...
			if(response.status){
				...
				$.confirm({
				    content: function(){
				        ...
				        return $.ajax({
								  type: "POST",
								  url: '{{ route('exercicio.create') }}',
								  data: $("#novo-exercicio").serialize(),
								  success: function(){
										$.alert({
										    title: 'Sucesso!',
										    content: 'Exercício cadastrado com sucesso!',
										    buttons:{
										    	ok:function(){
										    		window.location.href = "{{ route('professor.exercicio.index') }}";
										    	}
										    }
										});
								  }
		...
		$.confirm({
		    ...
		    buttons: {
		        Sim: function(helloButton){
		            document.location.href = "{{ route('professor.exercicio.index') }}"
		        },
		        Nao:{
		        	text:"Não"
		        }
