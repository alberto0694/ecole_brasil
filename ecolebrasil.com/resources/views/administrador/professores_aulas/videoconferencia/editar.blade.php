...
<form id="editar-videconferencia" class="form-horizontal" role="form" style="margin-left: 20px">
	{{ csrf_field() }}
	...
	<div class="form-group">
		...
		<div class="col-sm-6">
			<input value="{{ $video->titulo }}" id="titulo" name="titulo" type="text" class="col-xs-12 col-sm-6" />
		</div>
	</div>
	...
	<div class="form-group">
		...
		<div class="col-sm-6">
			<select id="modulo_id" name="modulo_id" class="col-xs-12 col-sm-6" id="form-field-select-3">
					...
					@foreach($video->modulo->agenda->modulos as $modulo)
						@if($video->modulo->id == $modulo->id)
...
jQuery(function($){
	...
	//VALIDATOR JQUERY
	$("#editar-videconferencia").validate({
		rules: {},
		messages: {},
			...
			if(response.status){
				...
				$.confirm({
				    content: function(){
				        ...
				        return $.ajax({
								  type: "POST",
								  url: '{{ route('videoconferencia.update', $video->id) }}',
								  data: $("#editar-videconferencia").serialize(),
								  success: function(){
										$.alert({
										    title: 'Sucesso!',
										    content: 'Videconferência cadastrada com sucesso!',
										    buttons:{
										    	ok:function(){
										    		window.location.href = "{{ route('administrador.videoconferencia.index') }}";
										    	}
										    }
										});
								  }
		...
		$.confirm({
		    ...
		    buttons: {
		        Sim: function(helloButton){
		            document.location.href = "{{ route('administrador.videoconferencia.index') }}"
		        },
		        Nao:{
		        	text:"Não"
		        }
