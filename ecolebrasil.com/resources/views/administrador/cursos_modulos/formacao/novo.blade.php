...
@section('content')
	...
	<form id="novo-formacao" class="form-horizontal" role="form" style="margin-left: 20px" >
		...
		{{-- <div class="form-group" >
			<div class="col-sm-9">
				<h4 class="header clearfix">Descrição</h4>
    ...
    jQuery(function($){
    	...
    	$("#novo-formacao").validate({
			...
			submitHandler: function(form) {
				let arr = [{ element:$("input[name=nome]"), type:'text' }];
			...
			$.confirm({
			    ...
			    buttons: {
			        Sim: function(helloButton){
			            document.location.href = "{{ route('administrador.formacao.index') }}"
			        },
			        Nao:{
			        	text:"Não"
			        }
